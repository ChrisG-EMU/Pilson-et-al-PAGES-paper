# ============================================================
# EAGL T-Score Descriptive Statistics
# ============================================================
...
# ---- File path ----
file_path <- path.expand("~/Desktop/EAGL T-score FINAL.xlsx")

# ---- Sheet definitions: name + exact cell ranges ----
# Data starts row 3; columns P:BK only
sheets <- list(
  list(name = "AC",                 range = "P3:BM101"),
  list(name = "HCP",                range = "P3:BM102"),
  list(name = "Gen Pop (EAGL val)", range = "P3:BM2710")
)
...
# BE=42, BF=43, BG=44, BH=45, BI=46, BJ=47, BK=48, BL=49, BM=50 -> Objective cols 34:50 (SUM)
...
local_cols <- list(
  ...
  Objective   = list(idx = 34:50, method = "sum")
)
...
for (sheet in sheets) {
  ...
  if (ncol(data_raw) != 50) {
    warning("Expected 50 columns for sheet '", sheet$name,
